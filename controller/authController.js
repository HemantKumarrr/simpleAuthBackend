...
module.exports.signup = async (req, res) => {
  try {
    ...
    res.cookie("jwt", token, { expires: new Date(new Date().getTime() + 100*1000), httpOnly: true, domain: 'simple-auth-frontend-one.vercel.app', sameSite: 'none' });
...
module.exports.logout = (req, res) => {
  try {
    res.cookie("jwt", "", { maxAge: 1000, httpOnly: true });
    res.send("logout");
  } catch (err) {
    res.send(err);
  }
